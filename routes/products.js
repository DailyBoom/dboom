...
router.post('/products/search', function(req, res) {
  Product.find({ extend: 4 }, '_id name brand price wholesale_price options options_skin w_cz_price w_eu_price').or([{ 'name': { $regex: req.body.term, $options: "i" } }, { 'brand': { $regex: req.body.term, $options: "i" } }]).exec(function(err, products) {
